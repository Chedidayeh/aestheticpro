...
interface fetchedCat extends Category {
  ...
  const CategoryView = ({ categories }: ProductViewProps ) => { 
    const t = useTranslations('AdminStockPage');
    ...
    const handleSizeDelete = async () =>{
        try {
          await deleteSize(sizeId)
            ...
            console.log(error)
            toast({
                title: t('toast_size_not_deleted'),
                variant: 'destructive',
              });
        }
    }
        ...
        try {
            ...
            toast({
                ...
                setOpen(true)
                    ...
                    console.log(error)
                    toast({
                        title: t('toast_color_not_deleted'),
                        variant: 'destructive',
                        });
                        setOpen(true)
                        }
    ...
    const handleSaveCategory = async () => {
      if (newSizes[0].label  ==="" && colors[0].label ==="" ) {
        toast({
          title: t('toast_missing_required_fields'),
          description: t('toast_fill_required_fields'),
          variant: "destructive",
        });
        return;
      }
