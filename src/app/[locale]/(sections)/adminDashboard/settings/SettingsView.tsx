    ...
    const [updatedPlatformData, setUpdatedPlatformData] = useState({
      ...
      const handleAddLevel = async () => {
        try {
          ...
          toast({
            title: t('toast_failed_to_add_level'),
            description: error instanceof Error ? error.message : t('toast_unknown_error'),
            variant: 'destructive',
          });
        }
      };
      ...
      const handleDeleteLevel = async (levelId: number) => {
        try {
          ...
          // Error toast notification
          toast({
            title: t('toast_failed_to_delete_level'),
            description: error instanceof Error ? error.message : t('toast_unknown_error'),
            variant: 'destructive',
          });
        } finally {
      ...
      const handleAddCollection = async () => {
        try {
            ...
            if (res.success) {
                setOpen(false);
                toast({
                    title: t('toast_collection_added'),
                    variant: 'default',
                });
                router.refresh();
            } else {
                setOpen(false);
                toast({
                    title: t('toast_failed_to_add_collection'),
                    variant: 'destructive',
                });
                router.refresh();
            }
    ...
    const handleAddContent = async () => {
        try {
            ...
            if (res) {
                ...
                setOpen(false);
                toast({
                    title: t('toast_failed_to_add_content'),
                    variant: 'destructive',
                });
                router.refresh();
            }
    ...
    const handleDeleteCollection = async (collection: ExtraCollection) => {
      try {
          ...
          if (res.success) {
              setOpen(false);
              toast({
                  title: t('toast_collection_deleted'),
                  variant: 'default',
              });
              router.refresh();
          } else {
              setOpen(false);
              toast({
                  title: t('toast_failed_to_delete_collection'),
                  variant: 'destructive',
              });
              router.refresh();
          }
