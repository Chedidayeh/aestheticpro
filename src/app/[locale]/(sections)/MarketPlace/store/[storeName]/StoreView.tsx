...
const StoreView = ({ initialProducts,totalCount,initialPage, limit, priceRanges,store, user ,storeProductsCount, designs , categories , collections , followersCount , follows }: ProductReelProps) => {
  ...
  <p className="mt-2 text-sm text-muted-foreground">
    {store?.storeBio}
     ...
     <div className='mt-4'>
      <p className="text-sm font-medium">{t('followers', { count: followersCount })}</p>
    </div>
    ...
    <div className={`mt-4 ${isFollowing ? '' : 'animate-wiggle'}`}>
      ...
      <DropdownMenuTrigger asChild>
        ...
        }`} variant="outline">{loading ? t('loading') : t('following') } <ChevronDown size={16} /></Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent side='bottom' className="w-18">
        <DropdownMenuLabel className='text-xs text-muted-foreground'>{t('unfollow_store')}</DropdownMenuLabel>
        <DropdownMenuSeparator />
        <DropdownMenuGroup>
      ...
      >
        {loading ? t('loading') : t('follow_store')}
      </Button> 
...
<div className="mt-4 items-center justify-center flex">
        ...
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="Products">{t('store_products')}</TabsTrigger>
          <TabsTrigger value="Designs">{t('store_designs')}</TabsTrigger>
        </TabsList>
      </Tabs>
    ...
    {store.displayContact && (
      ...
      <section className='border rounded-2xl w-[50%] bg-muted/50 border-gray-200'>
        <MaxWidthWrapper className='py-4 text-center'>
          <div className='my-2'>
            <p className='text-sm'>{t('contact_seller')}</p>
          </div>
