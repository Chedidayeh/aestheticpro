...
const DesignConfigurator: React.FC<DesignConfiguratorProps> = ({ SellersDesignsData, selectedCategory, categories, platform, user }) => {
  const t = useTranslations('MarketPlaceDesignConfigurator');
  ...
  const handleFileChange = (file: File) => {
    if (file) {
      if (file.size > MAX_FILE_SIZE) {
        ...
        toast({
          title: t('file_size_exceeds'),
          description: t('please_choose_file_equal_or_smaller'),
          variant: 'destructive',
        });
        return
          ...
          if (e.target) {
            ...
            image.onload = () => {
              ...
              if (width >= 1000 && width <= 4000 && height >= 1000 && height <= 4000) {
                ...
                setFrontDesignFile(undefined)
                toast({
                  title: t('invalid_front_design_dimensions'),
                  description: t('please_upload_design_with_width_and_height_between_1000px_and_4000px'),
                  variant: 'destructive',
                });
                return
              }
          ...
          if (e.target) {
            ...
            image.onload = () => {
              ...
              if (width >= 1000 && width <= 4000 && height >= 1000 && height <= 4000) {
                ...
                setBackDesignFile(undefined);
                toast({
                  title: t('invalid_back_design_dimensions'),
                  description: t('please_upload_design_with_width_and_height_between_1000px_and_4000px'),
                  variant: 'destructive',
                });
                return
  ...
  const uploadDesign = async (file: File) => {
    ...
    try {
      ...
      toast({
        ...
        description: t('error_uploading_image'),
        variant: 'destructive',
      });
    }
  }
  ...
  const uploadCapturedMockup = async (file: File) => {
    ...
    try {
      ...
      toast({
        ...
        description: t('error_uploading_product_image'),
      });
  ...
  const savePreOrder = async () => {
    try {
      ...
      console.error(error);
      setOpenDialog(false)
      showToast(t('error'), t('please_try_again_later'), 'destructive');
  ...
  const handleSaveResult = (result: {
    ...
    if (result.success) {
      showToast(t('great'), t('preorder_saved_successfully'), 'default');
      router.push("/MarketPlace/create-client-product/preview?preOrderId=" + result.preOrderId);
    } else {
      setOpenDialog(false)
      showToast(t('error'), t('failed_to_save_preorder'), 'destructive');
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:grid-cols-1">
          ...
          <Card x-chunk="dashboard-05-chunk-3" className={cn(' lg:rounded-2xl shadow-lg max-h-max')}>
            ...
            <CardContent className="items-center space-y-6 grid" >
              ...
              {/* category selection */}
              <h3>{t('change_category')}</h3>
              <div className="ml-5">
                <Sheet open={openSheet} onOpenChange={setOpenSheet}>
                  <SheetTrigger asChild>
                    <Button variant="secondary">{t('select')}</Button>
                  </SheetTrigger>
              ...
              {(addFrontDesign || addBackDesign) && (
                ...
                <>
                  ...
                  <div className='space-y-2'>
                    <h3>{t('upload_a_design')}</h3>
                    <p className='text-xs text-zinc-500 ml-5'>{t('file_types')}</p>
                    <p className="text-xs text-zinc-500 ml-5">{t('recommended_size')}</p>
                    <p className="text-xs text-zinc-500 ml-5">{t('acceptable_range')}</p>
                    <p className='text-xs text-zinc-500 ml-5'>{t('client_design_price', { price: platform.clientDesignPrice.toFixed(2) })}</p>
                  ...
                  {SellersDesignsData.length > 0 && (
                    <>
                      <h3>{t('or_choose_existing')}</h3>

                      <div className="flex items-center justify-center">
                        <Button variant="secondary" size="sm" className="w-[40%] " onClick={handleToggleMode}>
                          {t('toggle_mode')}
                        </Button>
                      </div>
                      <div>
                        <Input type="text" placeholder={t('search_placeholder')} value={searchQuery} onChange={handleSearchChange} />
                      </div>
                  ...
                  <div className="flex-1 text-center">
                    <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
                      <PopoverTrigger asChild>
                        <Button variant="default" className="text-white" onClick={() => setPopoverOpen(true)}>
                          {t('create_order')}
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-[95vw] max-w-md sm:max-w-lg lg:max-w-xl xl:max-w-2xl mx-2">
                        <div className='flex flex-col gap-4 sm:gap-6 px-3 sm:px-4 py-4 sm:py-6'>
                          ...
                          >
                            <Label className="text-sm sm:text-base">{t('color_label', { color: options.color.label })}</Label>
                            <div className='mt-2 sm:mt-3 flex flex-wrap gap-1.5 sm:gap-2 justify-center sm:justify-start'>
                                ...
                                >
                                  <span
                                    className={cn(
                                      ...
                                      'h-6 w-6 sm:h-8 sm:w-8 rounded-full border border-black border-opacity-10'
                                    )}
                                  />
                                </RadioGroup.Option>
                              ))}
                          ...
                          <div className='relative flex flex-col gap-2 sm:gap-3 w-full'>
                            {/* Size */}
                            <Label className="text-sm sm:text-base">{t('size')}</Label>
                            <Select
                              value={options.size.value}
                              onValueChange={(value) => {
                                const size = selectedP.sizes.find((s) => s.value === value);
                                if (size) setOptions((prev) => ({ ...prev, size }));
                              }}
                            >
                            ...
                            {/* Quantity */}
                            <Label className="text-sm sm:text-base">{t('quantity_label', { min: 1, max: platform.maxProductQuantity })}</Label>
                            <Input 
                              type="number" 
                              value={quantity} 
                              onChange={updatePrice} 
                              min={1} 
                              max={platform.maxProductQuantity}
                          ...
                          <div className="text-center mt-3 sm:mt-4">
                            <Button
                              ...
                              {t('pass_to_order_confirmation')}
                              <ArrowRight className='h-4 w-4 sm:h-5 sm:w-5 ml-1 sm:ml-1.5 inline' />
                            </Button>

                            <div className="mt-3 sm:mt-4">
                              <Badge variant="outline" className="text-sm sm:text-base animate-borderPulse px-3 sm:px-4 py-1 sm:py-2">
                                <span className="animate-pulse">{t('total_label', { total: totalPrice.toFixed(2) })}</span>
                              </Badge>
                            </div>
                          </div>
                        </div>
          ...
          {(addFrontDesign || addBackDesign) && (
            ...
            <Card x-chunk="dashboard-05-chunk-3" className={cn(' lg:rounded-2xl shadow-lg max-h-max')}>
              <CardHeader className="px-7 flex flex-col items-center justify-center">
                ...
                >
                  <Label>{t('color_label', { color: options.color.label })}</Label>
              ...
              <CardContent >
                ...
                {addFrontDesign && (
                  ...
                  <>
                    <div className="text-center">
                      <h1 className='text-3xl font-extrabold'>{t('front_design')}</h1>
                    </div>
                    ...
                    <div className="text-center">
                      ...
                      >
                        {isBorderHidden ? t('show_border') : t('hide_border')}
                      </Button>
                    </div>
                ...
                {addBackDesign && (
                  ...
                  <>
                    <div className="text-center">
                      <h1 className='text-3xl font-extrabold'>{t('back_design')}</h1>
                    </div>
                    <div className='w-full h-px bg-zinc-200 my-5' />
                    ...
                    <div className="text-center">
                      ...
                      >
                        {isBackBorderHidden ? t('show_border') : t('hide_border')}
                      </Button>
                    </div>
                  </>
                )}
              ...
              <CardFooter className='relative flex flex-col items-center justify-center'>
                <div className="text-center">
                  <Label className='text-sm'>
                    <span className="text-blue-600 ">{t('guide')}</span>: {t('hold_design_to_drag')}
                  </Label>
                </div>
              </CardFooter>
            </Card>
