...
const SearchedProducts = ({ initialProducts,totalCount,initialPage, limit,priceRanges , user , categories , collections , searchQuery }: ProductReelProps) => {
  ...
  const t = useTranslations('SearchedProductsPage');
  ...
  const handleSortChange = async (event: string) => {
    try {
      ...
      toast({
        ...
        description: t('toast_error_sorted'),
        variant: "destructive",
      });
    } finally {
  ...
  const handleCategorySortChange = async (event: string) => {
    try {
      ...
      toast({
        title: t('toast_error_title'),
        description: t('toast_error_category'),
        variant: "destructive",
      });
    } finally {
      setOpen(false); // Hide loading indicator
  ...
  const handleCollectionSortChange = async (event: string) => {
    try {
      ...
      toast({
        title: t('toast_error_title'),
        description: t('toast_error_collection'),
        variant: "destructive",
      });
    } finally {
      setOpen(false); // Hide loading indicator
  ...
  const handlePriceRangeChange = async (value: string) => {
    try {
      ...
      toast({
        title: t('toast_error_title'),
        description: t('toast_error_price_range'),
        variant: "destructive",
      });
    } finally {
      setOpen(false); // Hide loading indicator
  ...
  const handlePageChange = async (page: number) => {
    try {
      ...
      toast({
        title: t('toast_error_title'),
        description: t('toast_error_page'),
        variant: "destructive",
      });
    } finally {
      setOpen(false); // Hide loading indicator
  ...
  return (
    <section className='py-4'>
     ...
     <h1 className='text-3xl font-bold tracking-tight md:text-5xl'>
        {t('searched_products_title', {query: searchQuery})}
      </h1>   
