...
<>
    ...
    <MaxWidthWrapper>
      ...
      <div className='mt-8 lg:col-end-2 lg:row-span-2 lg:self-center'>
        ...
        <div className="my-4 grid grid-cols-2 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-2">          
          ...
          <div className="flex justify-between items-center sm:mt-10 lg:mt-4">
            {/* <ol className="flex items-center space-x-2">
             ...
             <div className='flex items-center mt-2'>
                <div className='text-muted-foreground border-gray-300 '>
                  {t('store_label')} <span className='cursor-pointer hover:text-blue-700'>
                    <Link
                ...
                <div className='ml-4 border-l text-muted-foreground border-gray-300 pl-4'>
                  {t('category_label')} <span className='cursor-pointer hover:text-blue-700'>
                    <Link
                    className='animate-pulse font-bold text-blue-500'
            ...
            <div className="my-4 flex justify-start items-center md:justify-between"> {/* Adding margin bottom for separation */}
                <ViewDesign frontDesign={frontDesign} backDesign={backDesign}/>
                <div className="hidden md:flex items-center justify-end flex-col">
              <Link href={`/MarketPlace/create-client-product/select-category`} >
                <Button className='hover:text-blue-500' size={"sm"} variant="secondary">{t('try_other_designs')} →</Button>
              </Link>
            </div>
              </div>
            ...
            {product.NewProduct && (
                ...
                <Badge variant="outline" className="bg-blue-700 text-white">{t('badge_new')}</Badge>
              </div>
                )}
                 {product.topSales && (
                <div className=""> {/* Added margin for separation */}
                <Badge variant="outline" className="bg-emerald-700 text-white">{t('badge_best_sell')}</Badge>
              </div>
                )}
                 {product.isDiscountEnabled && (
                  <>
                <div className=""> {/* Added margin for separation */}
                <Badge variant="outline" className="bg-red-700 text-white">{t('badge_discount', {discount: product.discount!})}</Badge>
              </div>
              </>
                )}
          ...
          <div className="font-bold rounded-xl text-gray-400 text-md line-through">
            {(product.oldPrice ?? product.price  ).toFixed(2)} TND
                ...
                <Badge className='text-lg text-white font-bold'>
                {t('product_price')} {(product.price).toFixed(2)} TND 
              </Badge>
                </p>
              </div>
              ...
              <Label htmlFor="username" className="text-left">{t('select_size')}</Label>
             <div className='ml-3'>
             <Select value={selectedSize} onValueChange={handleSizeChange}>
            <SelectTrigger className="w-[180px]">
              <SelectValue placeholder={t('select_size_placeholder')} />
            </SelectTrigger>
            <SelectContent>
              <SelectGroup>
              ...
              <div className='mt-6 flex items-center'>
          <Label htmlFor="username" className="text-left">{t('select_color')}</Label>
          ...
          <div className="ml-3 w-full"> {/* Set full width for responsive layout */}
            ...
            >
              ...
              <div className='mt-6 flex items-center'>
              <Label htmlFor="username" className="text-left">{t('select_quantity')}</Label>
             ...
             <SquareMinus 
              ...
              <SelectTrigger className="w-[70px]">
                <SelectValue placeholder={t('select_quantity_placeholder')} />
              </SelectTrigger>
            ...
            <div className="mt-6">
              <p className="text-gray-400 text-sm">{t('scroll_down_dimensions')}</p>
              <ArrowDown className="text-gray-400 h-5 animate-bounce"/>
                      ...
                      {/* add to cart part */}
          <div className='mt-2 lg:col-start-1 lg:row-end-2 lg:max-w-lg lg:self-start'>
            <div>
            <div className='flex items-center justify-center'>
                <p className=''>
                <Badge variant={"secondary"} className='text-lg font-bold'>
                {t('product_total_price')} {(product.price * quantity).toFixed(2) } TND 
              </Badge>
                </p>
              </div>
              ...
              <Link href={"/MarketPlace/cart"}
               className="flex justify-center text-sm text-green-500 animate-pulse items-center">
                {t('view_cart')} &rarr;
              </Link>
              </div>
              ...
              <Link href={`/MarketPlace/create-client-product/select-category`} >
                <Button size={"sm"} className='hover:text-blue-500' variant="secondary">{t('try_other_designs')} →</Button>
              </Link>
            </div>
      ...
      <ProductDimenetions
        title={t('product_dimensions_title')}
        subtitle={t('product_dimensions_subtitle')}
      />
      ...
      <ProductReel
        user={user!}
        href={`/MarketPlace/category/${product.category}`}
        title={t('similar_products_title', {category: product.category})}
        products={categoryProducts}
        subtitle={t('similar_products_subtitle', {category: product.category, title: product.title})}
      />
    </MaxWidthWrapper>
