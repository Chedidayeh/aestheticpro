...
const Reviews = (props: ProductReelProps) => {
  ...
  const createReview = async () => {
    if (!user) {
      ...
      toast({
        title: t('toast_no_user_title'),
        description: t('toast_no_user_desc'),
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      if (newReview) {
        toast({
          title: t('toast_review_success_title'),
          description: t('toast_review_success_desc'),
          variant: 'default',
          duration: 5000,
        });
        ...
        setOpen(false);
        toast({
          title: t('toast_review_error_title'),
          description: t('toast_review_error_desc'),
          variant: 'destructive',
          });
      }
    } catch (error) {
  ...
  const handleRemoveReview = async () => {
    try {
      ...
      if (response) {
        ...
        toast({
          title: t('toast_review_remove_success'),
          variant: 'default',
          duration: 3000,
        });
        setVisibleReviews((prevReviews) => 
        ...
        toast({
          title: t('toast_review_remove_failed_title'),
          description: t('toast_review_remove_failed_desc'),
          variant: 'destructive',
          duration: 3000,
        });
        setOpen(false);
      ...
      toast({
        title: t('toast_review_remove_error_title'),
        description: t('toast_review_remove_error_desc'),
        variant: 'destructive',
        duration: 3000,
      });
      setOpen(false);
  ...
  <div className='mt-6 flex items-center w-full'>
    ...
    <Alert className="w-full">
        ...
        <AlertTitle>{t('add_review')}</AlertTitle>
    ...
    <Button disabled={content.length===0 || content.length>100} onClick={createReview} size={"sm"} className="text-white" >{t('review_add_button')}</Button>
    </div>
    </AlertDescription>
    </Alert>
...
{visibleReviews.length < productReviews.length && (
  <div className="text-center mt-2">
    ...
    <AlertDialogHeader>
            <AlertDialogTitle>{t('review_delete_title')}</AlertDialogTitle>
            <AlertDialogDescription>
              {t('review_delete_desc')}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setOpenDeleteDialog(false)}>{t('review_delete_cancel')}</AlertDialogCancel>
            <AlertDialogAction 
