    ...
    setAddress(event.target.value);
                  ...
                  const handlePhoneNumberBlur = () => {
                    if (phoneNumber!.length !== 8) {
                      setPhoneNumberError(t('toast_phone_length'));
                    } else {
                      setPhoneNumberError('');
                    }
                  };
                  ...
                  const createOrder = async () => {
                    try {
                      ...
                      if(result.orderId && result.success){
                        ...
                        setOpenDialog(false)
                        toast({
                          title: t('toast_order_error_title'),
                          description: t('toast_order_error_desc'),
                          variant: 'destructive',
                      });
                      router.refresh()
                      }
...
const removeItem = async (cartProductId : string) =>{
  ...
  try {
    ...
    toast({
      ...
      description: t('toast_remove_error_desc'),
      variant: 'destructive',
    });
    return
...
useEffect(() => {
    ...
    <>
      ...
      <div className='flex items-center justify-between'>
        ...
        <div className='left-2 mt-4 justify-center items-center flex'>
          <p className='text-md '>{t('total_items', {count: cartProducts.length})}</p>
        </div>
  ...
  >
    {cartProducts.length === 0 ? (
      <div className='flex h-full flex-col items-center justify-center space-y-1'>
        ...
        <h3 className='font-semibold text-2xl'>{t('cart_empty')}</h3>
        <p className='text-muted-foreground text-center'>{t('nothing_to_show')}</p>
      ...
      <Table >
              ...
              <TableHeader>
                  <TableRow>
                  <TableHead>{t('product_image')}</TableHead>
                  <TableHead>{t('product_title')}</TableHead>
                  <TableHead>{t('product_price')}</TableHead>
                  <TableHead>{t('product_quantity')}</TableHead>
                  <TableHead>{t('product_size')}</TableHead>
                  <TableHead>{t('product_color')}</TableHead>
              ...
              {cartProducts.map((product) => (
                <TableRow key={product.cartProductId}>
                  ...
                  <TableCell>
                    <Link href={`/MarketPlace/product/${product.productId}`} className="font-medium hover:text-muted-foreground">
                      {product.title}
                    </Link>
                  </TableCell>
                  <TableCell>
  ...
  <section className='mt-16 rounded-lg border-2 px-4 py-6 sm:p-6 lg:col-span-5 lg:mt-0 lg:p-8'>
    <h2 className='text-lg font-medium'>{t('order_summary')}</h2>
    ...
    <div className='mt-6 space-y-4'>
      ...
      <div className='flex items-center justify-between border-t border-gray-200 pt-4'>
        <div className='flex text-sm text-muted-foreground'>
          <p className='flex space-x-2 text-sm '>
            <Check className='h-5 w-5 flex-shrink-0 text-green-500' />
            <span>{t('free_shipping', {limit: platform.freeShippingFeeLimit.toFixed(2)})}</span>
          </p>
        </div>
      </div>

      <div className='flex items-center justify-between border-t border-gray-200 pt-4'>
        <div className='text-base font-medium '>{t('order_total')}</div>
...
<AlertDialog open={isConfirmOpen}>
      ...
      <AlertDialogHeader>
          <AlertDialogTitle>{t('confirm_order_title')}</AlertDialogTitle>
          <AlertDialogDescription>
        ...
        <div className="grid gap-4 py-4">
          ...
          <div className="grid grid-cols-4 items-center gap-4">
            ...
            <Input id="address" placeholder={t('address_placeholder')} maxLength={20} type="text" className="col-span-3" value={address} onChange={handleAddressChange} />
          </div>
        </div>
        <AlertDialogFooter>
