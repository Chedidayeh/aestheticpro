...
const CreateDesignView = ({platform , store}: ProductViewProps) => {  
    const t = useTranslations('SellerCreateDesignPage');
      ...
      const handleFileChange = (file : File) => {
        if (file) {
          if (file.size > MAX_FILE_SIZE) {
            toast({
              title: t('toast_file_size_exceeds'),
              description: t('toast_file_size_limit'),
              variant: 'destructive',
            });
            setFile(null)
            return
              ...
              if (e.target) {
                ...
                image.onload = () => {
                  ...
                  if (width >= 1000 && width <= 4000 && height >= 1000 && height <= 4000) {
                    ...
                    setFile(null)
                    toast({
                      title: t('toast_invalid_dimensions'),
                      description: t('toast_dimension_range'),
                      variant: 'destructive',
                    });
                    return
                ...
                <Card x-chunk="dashboard-05-chunk-3" className={cn(' lg:rounded-2xl shadow-lg')}>
                        ...
                        <div className="flex justify-center">
                          ...
                          />
                              ...
                              <>
                              <h3>{t('step_details')}</h3>
                              <div className="ml-5 mt-3 text-blue-800">

                              <div className='flex'>
                                <FolderPen className="h-4 w-4"/>
                                <Label className='ml-2'>{t('design_name')}</Label>
                              </div>
                            ...
                            <div className='flex items-center'>
                              ...
                              <Label className='ml-2'>{t('tags')} <p className='text-xs text-zinc-500'>({t('optional')})</p></Label>
                              <p className='text-xs text-zinc-500 ml-5'>{t('enter_to_add')}</p>
                            </div>
                            <div className='mt-4 mb-4'>
                              <Input
                                disabled={!isDesignUploaded}
                            ...
                            <div>
                              {tags.length > 0 && (
                            <p className='text-xs text-zinc-500 ml-5'>{t('click_to_remove')}</p>
                              )}
                              {tags.map((tag, index) => (
                                <div
                                  key={index}
                              ...
                              <div className='flex'>
                                ...
                                <Label className='ml-2'>{t('design_price')}</Label>
                                {designPrice <= platform.maxDesignSellerProfit && designPrice >= platform.platformDesignProfit + 1 && (
                                  <>
                                <p className='text-xs text-zinc-500 ml-5'>{t('price_display', {price: designPrice.toFixed(2)})}</p>
                                <p className='text-xs text-zinc-500 ml-5'>{t('profit_display', {profit: sellerProfit.toFixed(2)})}</p>
                                </>
                              )}
                              </div>                              
                              <div className="mt-4 mb-4">
                                ...
                                disabled={!isDesignUploaded}
                                placeholder={t('placeholder_price', {max: platform.maxDesignSellerProfit, min: platform.platformDesignProfit + 1})}
