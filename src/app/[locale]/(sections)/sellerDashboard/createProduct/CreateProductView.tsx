...
const CreateProductView = ({categories , platform , store ,  collections}: ProductViewProps) => {  
  ...
  const t = useTranslations('SellerCreateProductPage');
  ...
  const handleSelectChange = (value: string) => {
    ...
    if (matchingCollection) {
      ...
      toast({
        title: t('collectionNotFoundTitle'),
        description: t('collectionNotFoundDescription', { value }),
        variant : "destructive"
      });
    }
  };
  ...
  const handleCatClick = (index: number, category: fetchedCat) => {
    if (category.disableCategory) {
      toast({
        title: t('unavailableCategoryTitle'),
        description: t('unavailableCategoryDescription'),
        variant: "destructive",
      });
...
const handleFileChange = (file : File) => {
  if (file) {
    if (file.size > MAX_FILE_SIZE) {
      ...
      toast({
        title: t('fileSizeExceedsLimitTitle'),
        description: t('fileSizeExceedsLimitDescription'),
        variant: 'destructive',
      });
        ...
        if (e.target) { 
          ...
          image.onload = () => {
            ...
            if (width >= 1000 && width <= 4000 && height >= 1000 && height <= 4000) {
              ...
              setFrontDesignFile(undefined)
              toast({
                title: t('invalidFrontDesignDimensionsTitle'),
                description: t('invalidFrontDesignDimensionsDescription'),
                variant: 'destructive',
              });
              return
        ...
        reader.onload = (e) => {
          if (e.target) { 
            ...
            if (width >= 1000 && width <= 4000 && height >= 1000 && height <= 4000) {
              ...
              setBackDesignFile(undefined);
              toast({
                title: t('invalidBackDesignDimensionsTitle'),
                description: t('invalidBackDesignDimensionsDescription'),
                variant: 'destructive',
              });
              return
            }
              ...
              const uploadDesign = async (file: File) => {
                ...
                const SaveBothDesign = async () =>{
                  if (!FrontDesignFile || !BackDesignFile) {
                    toast({
                      title: t('noUploadedDesignsTitle'),
                      description: t('pleaseMakeSureToUploadBothFrontAndBackDesignTitle'),
                      variant: 'destructive',
                    });
                    return
                  }

                  if (!validateProductForm('both')) {
                    toast({
                      title: t('errorTitle'),
                  ...
                  try {
                    ...
                    toast({
                    title: t('errorTitle'),
                    description: t('failedToAddYourProductTitle'),
                    variant: 'destructive',
                    });
                    return
                    }
                    ...
                    if(result.success){
                      toast({
                        title: t('greatTitle'),
                        description: t('productAddedSuccessfullyTitle'),
                        variant: 'default',
                      }); 
                    ...
                    if(result.error){
                      ...
                      toast({
                        title: t('errorTitle'),
                        description: t('failedToAddProductTitle'),
                        variant: 'destructive',
                      });
                      setisAdding(false)
                      return
                ...
                return new Blob([byteArray], { type: mimeType })
                  ...
                  const handleSellerProfitBlur = () => {
                    if (sellerProfit > platform.maxProductSellerProfit || sellerProfit < 1 ) {
                      setSellerProfitError(`${t('maxSellerProfitError', { max: platform.maxProductSellerProfit, min: 1 })}`);
                    } else {
  ...
  const [frontDesignError, setFrontDesignError] = useState(false);
  const [backDesignError, setBackDesignError] = useState(false);
  const [sellerProfitFieldError, setSellerProfitFieldError] = useState(false);

  // Validation function
  const validateProductForm = (type: 'both' | 'front' | 'back') => {
    let valid = true;
    // Reset all errors
    setProductTitleError(false);
    setProductDescriptionError(false);
    setColorError(false);
    setFrontDesignError(false);
    setBackDesignError(false);
    setSellerProfitFieldError(false);

    if (productTitle.length === 0 || productTitle.length > 20) {
    ...
    if (sellerProfit > platform.maxProductSellerProfit || sellerProfit < 1 || sellerProfit === 0 || !sellerProfit) {
      setSellerProfitFieldError(true);
      valid = false;
    }
    if ((type === 'both' || type === 'front') && !FrontDesignFile) {
      setFrontDesignError(true);
      valid = false;
    }
    if ((type === 'both' || type === 'back') && !BackDesignFile) {
      setBackDesignError(true);
      valid = false;
    }
    return valid;
  };
    ...
    <>
                      ...
                      <AlertDialogHeader className="flex flex-col items-center">
                              ...
                              <AlertDialogTitle className="text-xl text-blue-700 font-bold text-center">
                                {t('savingYourProductTitle')}
                              </AlertDialogTitle>
                              <AlertDialogDescription className="flex flex-col items-center">
                                {t('savingYourProductDescription')}
          ...
          <Loader className="text-blue-700 h-[30%] w-[30%] animate-spin mt-3" />
                        ...
                        <p className="text-sm text-muted-foreground mb-2">{t('sellerDashboardCreateProduct')}</p>
                        <h1 className="text-2xl font-semibold">{t('createProductTitle')}</h1>
                ...
                <div className="grid grid-cols-1  lg:grid-cols-2 gap-6 md:grid-cols-1">
                     ...
                     <Card x-chunk="dashboard-05-chunk-3" className={cn(' lg:rounded-2xl shadow-lg max-h-max')}>
                        ...
                        <CardContent className="items-center space-y-6 grid" >                          
                            <h3>{t('chooseYourCategory')}</h3>

                            {/* category selection */}
                             <div className="ml-5">
                             <Sheet open={openSheet} onOpenChange={setOpenSheet}>
                            <SheetTrigger asChild>
                              <Button variant="secondary">{t('selectButton')}</Button>
                            </SheetTrigger>
                            <SheetContent side="bottom">
                              <SheetHeader>
                                <SheetTitle>{t('selectCategoryTitle')}</SheetTitle>
                                <SheetDescription>
                                </SheetDescription>
                              </SheetHeader>
                              ...
                              <>
                                ...
                                <Label htmlFor="front">{t('addFrontDesignLabel')}</Label>
                               </div>

                                {/* Switch component back */}
                              <div className="flex items-center mt-4 space-x-2">
                                <Switch disabled={isAdding} id="back" defaultChecked={addBackDesign} onClick={handleBackSwitchChange } />
                                <Label htmlFor="back">{t('addBackDesignLabel')}</Label>
                               </div>

                               {(addFrontDesign || addBackDesign)  && (
                                <>
                              <div className="space-y-2">
                                <h3>{t('uploadADesign')}</h3>
                                <p className="text-xs text-zinc-500 ml-5">{t('pngJpgJpegMaxSize')}</p>
                                <p className="text-xs text-zinc-500 ml-5">{t('recommendedDesignSize')}</p>
                                <p className="text-xs text-zinc-500 ml-5">{t('acceptableRange')}</p>
                              ...
                              <h3>{t('selectYourDesignCollection')}</h3>
                              ...
                              <Select value={selectedCollection.name} onValueChange={handleSelectChange}>
                                ...
                                <SelectContent>
                                  <SelectGroup>
                                    <SelectLabel>{t('collectionsLabel')}</SelectLabel>
                                ...
                                {/* select colors */}
                                <div className="space-y-2">
                        <h3>{t('selectYourProductColors')}</h3>
                        {colorError ? <p className='text-red-500 text-xs'>please choose a color !</p> : ""}
                          ...
                          >
                            {selectedP.colors.map((color: Color, index) => (
                              ...
                              >
                                ...
                                {/* Product details */}
                              <h3>{t('fillProductDetails')}</h3>
                              <div className="ml-5 mt-3 text-blue-800">

                              <div className='flex'>
                                <FolderPen className="h-4 w-4"/>
                                <Label className='ml-2'>{t('productTitleLabel')}</Label>
                              </div>
                              {productTitleError ? <p className='text-red-500 text-xs mt-1'>please choose a title !</p> : ""}
                              ...
                              <div className="mt-2 mb-4">
                                ...
                                placeholder={t('addUniqueNamePlaceholder')} 
                                type='text'/>
                              </div>
                              <div className='flex'>
                                <FileText className="h-4 w-4"/>
                                <Label className='ml-2'>{t('productDescriptionLabel')}</Label>
                              </div>      
                              {productDescriptionError ? <p className='text-red-500 text-xs mt-1'>please add a description !</p> : ""}
                              ...
                              <div className="mt-2 mb-4">
                                ...
                                maxLength={62}
                                className='border-blue-600  text-black bg-gray-100' 
                                placeholder={t('describeYourDesignPlaceholder')} 
                                type='text'/>
                              </div>
                            ...
                            <div>
                              {tags.length > 0 && (
                            <p className='text-xs text-zinc-500 ml-5'>{t('clickOnTagToRemove')}</p>
                              )}
                              {tags.map((tag, index) => (
                                <div
                                  key={index}
                              ...
                              <div className="mt-2 mb-4">
                                ...
                                disabled={(!isDesignUploaded && !isBackDesignUploaded)}
                                placeholder={t('maxSellerProfitPlaceholder', { max: platform.maxProductSellerProfit, min: 1 })}
                          ...
                          {addFrontDesign === true && addBackDesign === true && selectedCat != null && (
                            <>
                              <div className="flex items-center flex-col justify-center">
                                <div className="mb-2">   
                                  <Button
                                    loadingText={t('addingButtonLoadingText')}
                                    className='text-white'
                                    size='default'
                                    isLoading={isAdding}
                                    onClick={SaveBothDesign}
                                  >
                                    {t('addProductToStoreButton')}
                                    <span className="ml-1"><CircleCheckBig /></span>
                                  </Button>
                                </div>
                                <div>
                                  <p className='text-xs text-zinc-500'>
                                    {t('bothDesignsSelected')} + {platform.ExtraDesignForProductPrice.toFixed(2)} TND
                                  </p>
                                </div>
                              </div>
                            </>
...
<div className='space-y-2'>
                  ...
                  <CardHeader className="px-7 flex flex-col items-center justify-center">
                              ...
                              <h1 className='text-3xl font-extrabold'>{t('frontDesignTitle')}</h1>
                              </div>
                          ...
                          >
                            {isBorderHidden ? t('showBorderButton') : t('hideBorderButton')}
                          </Button>
                              </div>
                              ...
                              />
                                          ...
                                          {selectedFrontDesign && (
                                                  ...
                                                  <div className="flex items-center justify-center text-xs mt-4">
                                                    {t('useSliderToControlDesignSize')}
                                                  </div>
              ...
              <CardFooter className='relative flex flex-col items-center justify-center'>
                              <div className="text-center text-2xl">
                                  <Label className='text-sm'>
                                    <span className="text-blue-600 ">{t('guide')}</span>: {t('holdDesignToDrag')}
                                  </Label>
                              </div>
                  </CardFooter>
                  </Card>
                      ...
                      {addBackDesign !== false && selectedCat !== null  && ( 
                        ...
                        <CardHeader className="px-7 flex flex-col items-center justify-center">
                              {selectedCat !== null && (
                                <>
                                  <div className="text-center">
                                  <h1 className='text-3xl font-extrabold'>{t('backDesignTitle')}</h1>
                                  </div>
                          ...
                          >
                            {isBackBorderHidden ? t('showBorderButton') : t('hideBorderButton')}
                            </Button>
                                  </div>
